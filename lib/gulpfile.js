    ...
    through2.obj(function z(file, encoding, next) {
      ...
      if (file.path.match(/(\/|\\)style(\/|\\)index\.js/)) {
        ...
        if (content.indexOf("'react-native'") !== -1) {
          // actually in antd-goo-mobile@2.0, this case will never run,
...
function publish(tagString, done) {
  let args = ['publish', '--with-antd-goo-tools', '--access=public'];
  if (tagString) {
  ...
  gulp.series(done => {
    ...
    if (npmArgs) {
      for (let arg = npmArgs.shift(); arg; arg = npmArgs.shift()) {
        if (/^pu(b(l(i(sh?)?)?)?)?$/.test(arg) && npmArgs.indexOf('--with-antd-goo-tools') < 0) {
          reportError();
          done(1);
          return;
        }
